...
def randomize(job):
    ...
    sim.run(10e3)
    hoomd.write.GSD.write(state=sim.state, mode='xb', filename=job.fn('random.gsd'))
...
def compress(job):
    ...
    hoomd.write.GSD.write(state=sim.state, mode='xb', filename=job.fn('compressed.gsd'))
    job.document['compressed_step'] = sim.timestep


@flow.FlowProject.operation
@flow.FlowProject.pre.after(compress)
@flow.FlowProject.post(lambda job: job.document.get('timestep', 0) - job.document['compressed_step'] >= N_EQUIL_STEPS)
